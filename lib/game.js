...
function onSocketConnection (client) {
  ...
  // Listen for shoot player message
  client.on('shoot player', onShootPlayer);
  
  // Listen for change player message
  client.on('change', onChangePlayer);
}
...
function onShootPlayer (data) {
  ...
  this.broadcast.emit('shoot player', { id: shootPlayer.id, x: shootPlayer.getX(), y: shootPlayer.getY(), angle: shootPlayer.getAngle() });
}
...
function onChangePlayer (data) {
  ...
  // Send the message
  this.broadcast.emit('change player', { id: changePlayer.id, key: data.key });
}
